...
public class Correntista {
	...
	public String getNome() {
	    try (Connection connection = istanza.getConnection()) {
	    	String query = "SELECT nome FROM correntista";
	...
	public String getCognome() {
	    try (Connection connection = istanza.getConnection()) {
	    	String query = "SELECT cognome FROM correntista";
	       
	        try (PreparedStatement statement = connection.prepareStatement(query)) {
	            statement.setString(1, getCognome());
	            try (ResultSet resultSet = statement.executeQuery()) {
	                if (resultSet.next()) {
	...
	public String getCitta() {
	    try (Connection connection = istanza.getConnection()) {
	    	String query = "SELECT citta FROM correntista";
	       
	        try (PreparedStatement statement = connection.prepareStatement(query)) {
	            statement.setString(1, getCitta());
	            try (ResultSet resultSet = statement.executeQuery()) {
	                if (resultSet.next()) {
	...
	public String getNazione() {
	    try (Connection connection = istanza.getConnection()) {
	    	String query = "SELECT nazione FROM correntista";
	       
	        try (PreparedStatement statement = connection.prepareStatement(query)) {
	            statement.setString(1, getNazione());
	            try (ResultSet resultSet = statement.executeQuery()) {
	                if (resultSet.next()) {
	...
	public String getTelefono() {
	    try (Connection connection = istanza.getConnection()) {
	    	String query = "SELECT telefono FROM correntista";
	       
	        try (PreparedStatement statement = connection.prepareStatement(query)) {
	            statement.setString(1, getTelefono());
	            try (ResultSet resultSet = statement.executeQuery()) {
	                if (resultSet.next()) {
